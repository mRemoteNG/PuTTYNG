...
| `0002-putty-h-hwnd-parent.patch` | `putty.h`, `windows/window.c` | Declares a pointer-safe `hwnd_parent` handle. The terminal window is still created as a normal top-level window (never with `WS_CHILD` at creation time, since Windows rejects that combination with error 1406 unless a real parent HWND is supplied). Once `GWLP_USERDATA` has been set, it is reparented under `hwnd_parent` via `SetParent` and has `WS_POPUPWINDOW \| WS_THICKFRAME` stripped from its style (it is *not* switched to `WS_CHILD`). It also tracks the foreground window via `hwndLastActive`/`GetGUIThreadInfo` and, on `WM_MOUSEACTIVATE`, calls `BringWindowToTop` on the root ancestor of `hwnd_parent` so focus passes back to the hosting app correctly; skips the scrollbar enable/disable resync when embedded; guards `WM_SETFOCUS`/`WM_MOVE`/`WM_SIZE` against `term` being NULL (these can arrive before `term_init` runs); and forces `WM_SIZE` to `SIZE_MAXIMIZED` plus `IsZoomed()` to true so the embedded window always behaves as maximised. This logic is ported from the pre-0.80 PuTTYNG implementation (last known-good at commit `b417acfe`), which is the approach mRemoteNG's embedding actually depends on. |
