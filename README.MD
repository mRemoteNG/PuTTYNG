...
| `putty.h` | patch | Declares `hwnd_parent`, forces `IsZoomed()` true so the embedded window behaves as maximised. |
| `windows/window.c` | patch | Writes errors into the terminal instead of a modal `MessageBox`, which would otherwise block an embedded session. |
